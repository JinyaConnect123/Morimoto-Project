...
public class Util {
	...
	public static Integer nullToInteger(String s) {
		return (s == null || s.trim().length() == 0) ? null : Integer.valueOf(s);
	}

	public static Double nullToDouble(String s) {
		return (s == null || s.trim().length() == 0) ? null : Double.valueOf(s);
	}
	
	public static BigDecimal nullToBigDecimal(String s) {
...
/*
	public static String shiftTimeZoneToTokyo(String strDate){
	    ...
	    int MM = Integer.parseInt(strDate.substring(5,7));
	    int dd = Integer.parseInt(strDate.substring(8,10));
	    int HH = Integer.parseInt(strDate.substring(11,13));
	    int mm = Integer.parseInt(strDate.substring(14,16));
	    int ss = Integer.parseInt(strDate.substring(17,19));
	    cal.clear();
	    cal.set(yyyy,MM-1,dd);
	    ...
	    rtnStrDate += String.valueOf(cal.get(Calendar.YEAR)) + "-";
	    rtnStrDate += StringUtils.leftPad(String.valueOf(cal.get(Calendar.MONTH)+1), 2, '0') + "-";
	    rtnStrDate += StringUtils.leftPad(String.valueOf(cal.get(Calendar.DATE)), 2, '0') + "T";
	    rtnStrDate += StringUtils.leftPad(String.valueOf(cal.get(Calendar.HOUR_OF_DAY)), 2, '0')+ ":";
	    rtnStrDate += StringUtils.leftPad(String.valueOf(cal.get(Calendar.MINUTE)), 2, '0')+ ":";
	    rtnStrDate += StringUtils.leftPad(String.valueOf(cal.get(Calendar.SECOND)), 2, '0') + ".000Z";
	...
	public static String shiftTimeZoneToLondon(String strDate){
	 ...
	 * @param e
	 */
	public static void errorTerminate(JinyaException e){
		logger.info("CALL IN:Util.class errorTerminate");
		// エラーロゴ書き込み
		logger.info(e.getMessage());
		// TODO:AlertMessage設定が必要

		logger.info("CALL OUT:Util.class errorTerminate");
	}
	/**
	 * 予期外エラーが発生する場合
	 * @param e
	 */
	public static void unexpectedTerminate(Exception e){
		logger.info("CALL IN:Util.class unexpectedTerminate");
		// エラーロゴ書き込み
		logger.info(e.getMessage());
		String msg = e.getMessage();
		String code = "999";
		logger.error("エラーコード：" + code + " メッセージ：" + e.getMessage());
		try {
			Runtime.getRuntime().exec("eventcreate /ID " + code +" /L system /SO Cmd /T Error /D \"" + msg +"\"");
			logger.debug("システムログへError送信－Exception");
		} catch (IOException e1) {
			e1.printStackTrace();
			logger.debug(e1.getMessage());
		}
		logger.info("CALL OUT:Util.class unexpectedTerminate");
	}
	
	public static Date getDateTime(String fmtDateString) {
		
		Calendar c = Util.toCalendar(fmtDateString);
		Date d = c.getTime();
